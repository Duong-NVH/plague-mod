    ...
    PausableUpgradeable
{
    ...
    function swapBack() internal swapping {
        ...
        uint256 _treasuryFeeValue = amountETHToTreasuryAndSIF.mul(treasuryFee).div(_denom);

        if (_treasuryFeeValue > 0) {
            (bool success, ) = payable(treasuryReceiver).call{
                value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(_denom),
                gas: 30000
            }("");
        }
        ...
        uint256 totalMetisFee = amountETHToTreasuryAndSIF.mul(metisFee).div(_denom);
