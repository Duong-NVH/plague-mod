...
contract NODERewardManagement {
    ...
    function _changeNodeName(address _account, uint256 _creationTime, string memory newName) external onlySentry {
         NodeEntity[] storage nodes = _nodesOfUser[_account];
        uint256 numberOfNodes = nodes.length;
        require(
            numberOfNodes > 0,
            "CHANGE NAME ERROR: You don't have nodes to change"
        );
...
contract LVT is ERC20, Ownable, PaymentSplitter {
    ...
    function changeNodeName(uint256 _creationTime, string memory newName) public {
        ...
        nodeRewardManager._changeNodeName(msg.sender, _creationTime, newName);
    }
